...
struct MatTransposeNodeTrait {
    using InputNodeValueType = typename InputNodeTypePara::ValueType;
    using type = typename ad_math::mat_transpose_trait<InputNodeValueType>::type;
};

template<typename InputNodeTypePara>
struct MatTransposePolicyDefault {
    using InputNodeValueType = typename InputNodeTypePara::ValueType;
    using type = typename ad_math::mat_transpose_policy<InputNodeValueType>;
};

template<typename InputNodeTypePara, unsigned int indexPara,
         typename ValueTypePara = typename MatTransposeNodeTrait<InputNodeTypePara>::type, 
         typename PolicyPara = typename MatTransposePolicyDefault<InputNodeTypePara>::type,
         bool legalPara = isNodeType<InputNodeTypePara>::value>
class MatTransposeNodeImp;

template<typename InputNodeTypePara, unsigned int indexPara, typename ValueTypePara, typename PolicyPara>
class MatTransposeNodeImp<InputNodeTypePara, indexPara, ValueTypePara, PolicyPara, true>
  : public GraphNodeBase<MatTransposeNodeImp<InputNodeTypePara, indexPara, ValueTypePara, PolicyPara,true>> {
  public:
    using Base = GraphNodeBase<MatTransposeNodeImp>;
    using InputNodeTypes = std::tuple<std::shared_ptr<InputNodeTypePara>>;
    using ValueType = ValueTypePara;
    using Policy = PolicyPara;
    using ConcreteNodeType = MatTransposeNodeImp;
    static constexpr unsigned int index() {
        return indexPara;
    }

    MatTransposeNodeImp() = delete;

    MatTransposeNodeImp(std::shared_ptr<InputNodeTypePara> inputNode)
      : Base(), inputs(InputNodeTypes(inputNode)) {
        output = ad_matTranspose(inputNode->getValue());
    }
...
struct traits<MatTransposeNodeImp<InputNodeTypePara, indexPara, ValueTypePara, PolicyPara, legalPara>> {
    using InputNodeTypes = std::tuple<std::shared_ptr<InputNodeTypePara>>;
    using ValueType = ValueTypePara;
    using Policy = PolicyPara;
...
template<typename ConcreteInputNodeTypePara, unsigned int indexPara = ConcreteInputNodeTypePara::index()>
auto matTransposeImp(const std::shared_ptr<ConcreteInputNodeTypePara>& inputNodePtr) {
    using NodeType = MatTransposeNodeImp<ConcreteInputNodeTypePara, indexPara>;
    return std::shared_ptr<NodeType>(new NodeType(inputNodePtr));
}

template<typename ConcreteInputNodeTypePara>
auto matTranspose(const NodeWrapper<ConcreteInputNodeTypePara>& inputNode) {
    auto concreteResultNodePtr = matTransposeImp(inputNode.pNode);
